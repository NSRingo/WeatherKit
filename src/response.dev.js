...
async function InjectCurrentWeather(url, body, Settings) {
	...
	switch (Settings?.CurrentWeather?.Provider) {
		...
		case "QWeather": {
			const qWeather = new QWeather({ url: url, host: Settings?.API?.QWeather?.Host, header: Settings?.API?.QWeather?.Header, token: Settings?.API?.QWeather?.Token });
			currentWeather = (await qWeather.WeatherNow()).currentWeather;
			break;
		}
		case "ColorfulClouds":
		default: {
			const colorfulClouds = new ColorfulClouds({ url: url, header: Settings?.API?.ColorfulClouds?.Header, token: Settings?.API?.ColorfulClouds?.Token || "Y2FpeXVuX25vdGlmeQ==" });
			currentWeather = await colorfulClouds.RealTime();
			break;
		}
	}
...
async function InjectForecastDaily(url, body, Settings) {
	...
	switch (Settings?.Daily?.Provider) {
		case "WeatherKit":
			{
				if (!body?.forecastDaily) {
					Settings.Daily.Provider = Settings.Daily.FallbackProvider;
					return await InjectForecastDaily(url, body, Settings);
				}
			}
			break;
		case "QWeather": {
...
async function InjectForecastHourly(url, body, Settings) {
	...
	switch (Settings?.Hourly?.Provider) {
		case "WeatherKit":
			{
				if (!body?.forecastHourly) {
					Settings.Hourly.Provider = Settings.Hourly.FallbackProvider;
					return await InjectForecastHourly(url, body, Settings);
				}
			}
			break;
		case "QWeather": {
		...
		case "WeatherKit":
			{
				if (!body?.forecastNextHour) {
					Settings.NextHour.Provider = Settings.NextHour.FallbackProvider;
					return await InjectForecastNextHour(url, body, Settings);
				}
			}
			break;
		case "QWeather": {
